...
def main():
    ...
    fdTrain=pd.read_csv("Alexnet_c5_hierarchical_model.csv", header=0).to_numpy()
    fdRepr=pd.read_csv("Alexnet_c5_representative_descriptors.csv", header=0).to_numpy()
    ...
    pd.DataFrame(zip(distances,times,neighbour), columns=['distance', 'cpu time', 'neighbour'],).to_csv("hierarchical_location_alexnet_c5.csv", index=None)
